...
import {IFarmBooster} from "../interfaces/IFarmBooster.sol";
...
contract StakingPool is Ownable, Pausable {
    ...
    IPiggyDexV2Farm public immutable piggyDexV2Farm;

    address public boostContract; // boost contract used in Farm.
     ...
     * @param _piggyDexV2Farm: Farm contract
    ...
    constructor(
        IERC20 _token,
        IPiggyDexV2Farm _piggyDexV2Farm,
        address _admin,
        address _treasury,
        address _operator,
        uint256 _pid
        ...
        token = _token;
        piggyDexV2Farm = _piggyDexV2Farm;
        admin = _admin;
        treasury = _treasury;
        operator = _operator;
        cakePoolPID = _pid;
    ...
    function updateBoostContractInfo(address _user) internal {
        if (boostContract != address(0)) {
            ...
            IFarmBooster(boostContract).onStakingPoolUpdate(
                _user,
                user.lockedAmount,
                lockDuration,
                totalLockedAmount,
    ...
    function unlock(
        address _user
    ) external onlyOperatorOrCakeOwner(_user) whenNotPaused {
        UserInfo storage user = userInfo[_user];
        require(
    ...
    function harvest() internal {
        uint256 pendingCake = piggyDexV2Farm.pendingReward(
            cakePoolPID,
            address(this)
        );
        if (pendingCake > 0) {
            uint256 balBefore = available();
            piggyDexV2Farm.withdraw(cakePoolPID, 0);
    ...
    function calculateTotalPendingCakeRewards() public view returns (uint256) {
        uint256 amount = piggyDexV2Farm.pendingCake(cakePoolPID, address(this));
        return amount;
    }
