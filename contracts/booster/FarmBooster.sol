...
import {IPiggyDexV2Farm} from "../interfaces/IPiggyDexV2Farm.sol";
import {IterableMapping, ItMap} from "../libraries/IterateMapping.sol";
...
contract FarmBooster is Ownable {
    ...
    address public immutable PiggyDexV2Farm;
    ...
    event Refresh(address indexed user, uint256 pid);
    event UpdateBoostFarms(uint256 pid, bool status);
    event ActiveFarmPool(address indexed user, uint256 pid);
    ...
    /// @param _token Staking token contract address.
    /// @param _stakingPool Cake Pool contract address.
    /// @param _v2 PiggyDexV2Farm contract address.
    ...
    modifier onlyStakingPool() {
        require(msg.sender == StakingPool, "onlyCakePool: Not cake pool");
        _;
    }
    ...
    function onStakingPoolUpdate(
        ...
        for (uint256 i = 0; i < itmap.keys.length; i++) {
            ...
            _updateBoostMultiplier(_user, pid, avgDuration);
        }
    ...
    /// @notice Update user boost multiplier in V2 pool
    /// @param _user user address.
    /// @param _pid pool id in MasterchefV2 pool.
    function updatePoolBoostMultiplier(
        address _user,
        uint256 _pid
    ) public onlyProxy(_user) {
        // if user not actived this farm, just return.
        if (!userInfo[_user].contains(_pid)) return;
        _updateBoostMultiplier(_user, _pid, avgLockDuration());
    }
    ...
    function activate(uint256 _pid) external {
        require(
            whiteList[_pid] && msg.sender != address(0),
            "activate: Not boosted farm pool"
        );
        _updateBoostMultiplier(msg.sender, _pid, avgLockDuration());

        emit ActiveFarmPool(msg.sender, _pid);
    }
    ...
    function deactive(uint256 _pid) external {
        ...
        if (itmap.data[_pid] > BOOST_PRECISION) {
            IPiggyDexV2Farm(PiggyDexV2Farm).updateBoostMultiplier(
                proxy,
                _pid,
                BOOST_PRECISION
            );
    ...
    /// @param _user user address.
    function activedPools(
        address _user
    ) external view returns (uint256[] memory pools) {
    ...
    function getUserMultiplier(
        ...
        return _boostCalculate(_user, _pid, avgLockDuration());
    }

    /// @notice cake pool average locked duration calculator.
    function avgLockDuration() public view returns (uint256) {
        uint256 totalStakedAmount = IERC20(StakingPool).balanceOf(StakingPool);

        uint256 totalLockedAmount = IStakingPool(StakingPool)
            .totalLockedAmount();

        uint256 pricePerFullShare = IStakingPool(StakingPool)
        ...
        uint256 boostedRatio = ((IStakingPool(StakingPool).totalShares() -
            flexibleShares) * BOOST_RATIO_PRECISION) / originalShares;
        if (boostedRatio <= BOOST_RATIO_PRECISION) return 0;

        uint256 boostWeight = IStakingPool(StakingPool).BOOST_WEIGHT();
        uint256 maxLockDuration = IStakingPool(StakingPool)
            .MAX_LOCK_DURATION() * BOOST_RATIO_PRECISION;
    ...
    function _updateBoostMultiplier(
        ...
        uint256 prevMultiplier = IPiggyDexV2Farm(PiggyDexV2Farm)
            .getBoostMultiplier(_user, _pid);
        uint256 multiplier = _boostCalculate(_user, _pid, _duration);
    ...
    /// @param _user user address.
    /// @param _pid pool id(PiggyDexV2Farm pool).
    /// @param _duration staking pool average locked duration.
    function _boostCalculate(
        ...
        (uint256 lpBalance, , ) = IPiggyDexV2Farm(PiggyDexV2Farm).userInfo(
            _pid,
            _user
        );
        ...
        ) = IStakingPool(StakingPool).userInfo(_user);
        ...
        (, , , IERC20 lp, , , , ) = IPiggyDexV2Farm(PiggyDexV2Farm).poolInfo(
            _pid
        );
        uint256 userLockedDuration = (lockEndTime - lockStartTime) /
            (3600 * 24); // days

        uint256 aB = (((lp.balanceOf(PiggyDexV2Farm) *
            userLockedAmount *
